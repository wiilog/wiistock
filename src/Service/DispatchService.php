        ...
        if ($recipientAbleToReceivedMail || $requesterAbleToReceivedMail || $sendReport) {
            ...
            $subject = ($status->isTreated() || $status->isPartial() || $sendReport)
                ? ($dispatch->getEmergency()
                    ? ["Demande", "Acheminements", "Emails", "Follow Nexter // Urgence : Notification de traitement d'une demande d'acheminement", false]
                    : ["Demande", "Acheminements", "Emails", "Follow Nexter // Notification de traitement d'une demande d'acheminement", false])
                : (!$isUpdate
                    ? ["Demande", "Acheminements", "Emails", "Follow Nexter // Création d'une demande d'acheminement", false]
                    : ["Demande", "Acheminements", "Emails", "Follow Nexter // Changement de statut d'une demande d'acheminement", false]);
            ...
            $row = [
                ...
                ...($withStatuses
                    ? Stream::from(array_keys($statusFieldsConfig))
                        ->map(static fn($statusId) => $dispatch["statusHistory_$statusId"])
                        ->toArray()
                    : []
                ),
            ];
