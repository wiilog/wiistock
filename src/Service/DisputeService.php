    ...
    public function sendMailToAcheteursOrDeclarant(Dispute $dispute, string $category, $isUpdate = false) {
        ...
        if (!empty($recipients)) {
            ...
            $subject = fn(string $slug) => (
                !$isUpdate
                    ? ["Traçabilité", "Arrivages UL", "Email litige", "Follow Nexter // Litige sur {1}", false, [
                        1 => $this->translation->translateIn($slug, ...$translatedCategory)
                    ]]
                    : ["Traçabilité", "Arrivages UL", "Email litige", "Follow Nexter // Changement de statut d'un litige sur {1}", false, [
                        1 => $this->translation->translateIn($slug, ...$translatedCategory)
                    ]]
            );
