...
//                TODO Adrien pourquoi language n'est pas lié à attachment
    ...
    public function item(EntityManagerInterface $entityManager,
        ...
        return $this->render("settings/category.html.twig", [
            ...
            "path" => $path,
            "values" => $this->customValues($entityManager, $settingsService),
        ]);
    }
    ...
    public function customValues(EntityManagerInterface $entityManager, SettingsService $settingsService): array {
            ...
            self::CATEGORY_TRACKING => [
                self::MENU_ROUNDS => fn() => [
                    ...
                    "transportRoundNeededNaturesToDrop" =>
                        Stream::from(explode(',', $settingsService->getValue($entityManager, Setting::TRANSPORT_ROUND_NEEDED_NATURES_TO_DROP)))
                            ->filter(fn(string $value) => $value)
                            ->keymap(fn(string $value) => [
                                $value, [
                                    "value" => $value,
    ...
    public function typeHeader(Request $request,
        ...
        if ($edit) {
            ...
            if ($categoryLabel === CategoryType::STOCK_EMERGENCY) {
                $data[] = [
                    "label" => " email",
                    "value" => $formService->macro("select", "stockEmergencyModes", null, false, [
                        "type" => "",
                        "multiple" => true,
                        "items" => [
                            [
                ...
                $data[] = [
                    ...
                    "value" => $formService->macro("input", "averageTime", null, false, $averageTime, [
                        "additionalAttributes" => [
                            ["name" => "pattern", "value" => DateTimeService::AVERAGE_TIME_REGEX],
                        ]
                    ]),
                ];
            }
