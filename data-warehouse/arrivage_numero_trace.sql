       ...
       IF(
           (
               ...
               (SELECT COUNT(*) FROM truck_arrival_line_arrivage WHERE truck_arrival_line_arrivage.truck_arrival_line_id = truck_arrival_line.id) = 0
                   ...
                   AND (
                   DATE(NOW()) > (
                       SELECT DATE_ADD(truck_arrival.creation_date, INTERVAL 1 DAY)
                       FROM truck_arrival
                       WHERE truck_arrival.creation_date < DATE_ADD(NOW(), INTERVAL 1 YEAR)
                         ...
                         AND truck_arrival.creation_date > truck_arrival.creation_date
                       ORDER BY truck_arrival.creation_date
                       LIMIT 1
                   )
                       OR (
                       DATE(NOW()) = (
