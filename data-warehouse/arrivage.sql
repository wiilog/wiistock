SELECT arrivage.id,
       ...
       (SELECT COUNT(pack_count.id)
        ...
        WHERE sub_arrivage.id = arrivage.id)                    AS nb_ul,
       GROUP_CONCAT(DISTINCT receivers.username SEPARATOR ', ') AS destinataires,
       ...
       type.label                                               AS type,
       GROUP_CONCAT(DISTINCT acheteurs.username SEPARATOR ', ') AS acheteurs,
