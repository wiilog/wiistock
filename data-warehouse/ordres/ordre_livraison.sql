...
FROM (
         SELECT ordre_livraison.id                                AS id,
                ...
                preparation_order_reference_line.quantity_to_pick AS quantite_a_livrer,
                ...
                IF(ordre_livraison.date_fin IS NOT NULL AND preparation.date IS NOT NULL,
                   ROUND(TIME_FORMAT(TIMEDIFF(ordre_livraison.date_fin, preparation.date), '%H')
                             ...
                             + TIME_FORMAT(TIMEDIFF(ordre_livraison.date_fin, preparation.date), '%s') / 3600, 4),
                   NULL)                                          AS delta_date
         ...
         FROM livraison AS ordre_livraison
                  ...
                  LEFT JOIN preparation_order_reference_line
                            ON preparation.id = preparation_order_reference_line.preparation_id
                  ...
                  LEFT JOIN reference_article ON preparation_order_reference_line.reference_id = reference_article.id
                  LEFT JOIN emplacement AS destination ON demande_livraison.destination_id = destination.id

         WHERE preparation_order_reference_line.picked_quantity > 0
         ...
         SELECT ordre_livraison.id                              AS id,
                ...
                destination.label                               AS emplacement,
                preparation_order_article_line.quantity_to_pick AS quantite_a_livrer,
         ...
         FROM livraison AS ordre_livraison
                  ...
                  LEFT JOIN utilisateur AS operateur ON ordre_livraison.utilisateur_id = operateur.id
                  LEFT JOIN demande AS demande_livraison ON preparation.demande_id = demande_livraison.id
                  LEFT JOIN preparation_order_article_line
                            ON preparation.id = preparation_order_article_line.preparation_id
                  LEFT JOIN article ON preparation_order_article_line.article_id = article.id
                  LEFT JOIN type ON demande_livraison.type_id = type.id
                  LEFT JOIN emplacement AS destination ON demande_livraison.destination_id = destination.id
                  LEFT JOIN utilisateur AS demandeur ON demande_livraison.utilisateur_id = demandeur.id
