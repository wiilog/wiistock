...
FROM (SELECT ((TIMESTAMPDIFF(SECOND, desired_date, validation_date)/86400)/7 + 1
    - (IF(DAYOFWEEK(validation_date) <> 2, 1, 0))
    - (SELECT count(*) FROM TEMP_non_worked_days WHERE jour BETWEEN validation_date AND desired_date AND DAYOFWEEK(jour) = 2)) AS NBLUN,
             (SELECT (
             ...
             ((TIMESTAMPDIFF(SECOND, desired_date, validation_date)/86400)/7 + 1
                 - (IF(DAYOFWEEK(validation_date) NOT IN (2, 3), 1, 0))
                 - (IF(DAYOFWEEK(desired_date) = 2, 1, 0))
                 - (SELECT count(*) FROM TEMP_non_worked_days WHERE jour BETWEEN validation_date AND desired_date AND DAYOFWEEK(jour) = 3)) AS NBMAR,
             (SELECT (
                         SELECT IF(horaire1 IS NOT NULL AND horaire2 IS NOT NULL,
                                   (TIME_TO_SEC(CAST(horaire2 AS TIME)) - TIME_TO_SEC(CAST(horaire1 AS TIME))), 0)
                         FROM TEMP_worked_days
             ...
             ((TIMESTAMPDIFF(SECOND, desired_date, validation_date)/86400)/7 + 1
                 - (IF(DAYOFWEEK(validation_date) NOT IN (2, 3, 4), 1, 0))
                 - (IF(DAYOFWEEK(desired_date) IN (2, 3), 1, 0))
                 - (SELECT count(*) FROM TEMP_non_worked_days WHERE jour BETWEEN validation_date AND desired_date AND DAYOFWEEK(jour) = 4)) AS NBMER,
             (SELECT (
                         SELECT IF(horaire1 IS NOT NULL AND horaire2 IS NOT NULL,
                                   (TIME_TO_SEC(CAST(horaire2 AS TIME)) - TIME_TO_SEC(CAST(horaire1 AS TIME))), 0)
                         FROM TEMP_worked_days
             ...
             ((TIMESTAMPDIFF(SECOND, desired_date, validation_date)/86400)/7 + 1
                 - (IF(DAYOFWEEK(validation_date) IN (6, 7, 1), 1, 0))
                 - (IF(DAYOFWEEK(desired_date) IN (2, 3, 4), 1, 0))
                 - (SELECT count(*) FROM TEMP_non_worked_days WHERE jour BETWEEN validation_date AND desired_date AND DAYOFWEEK(jour) = 5)) AS NBJEU,
             (SELECT (
                         SELECT IF(horaire1 IS NOT NULL AND horaire2 IS NOT NULL,
                                   (TIME_TO_SEC(CAST(horaire2 AS TIME)) - TIME_TO_SEC(CAST(horaire1 AS TIME))), 0)
                         FROM TEMP_worked_days
             ...
             ((TIMESTAMPDIFF(SECOND, desired_date, validation_date)/86400)/7 + 1
                 - (IF(DAYOFWEEK(validation_date) IN (7, 1), 1, 0))
                 - (IF(DAYOFWEEK(desired_date) NOT IN (6, 7, 1), 1, 0))
                 - (SELECT count(*) FROM TEMP_non_worked_days WHERE jour BETWEEN validation_date AND desired_date AND DAYOFWEEK(jour) = 6)) AS NBVEN,
             (SELECT (
                         SELECT IF(horaire1 IS NOT NULL AND horaire2 IS NOT NULL,
                                   (TIME_TO_SEC(CAST(horaire2 AS TIME)) - TIME_TO_SEC(CAST(horaire1 AS TIME))), 0)
                         FROM TEMP_worked_days
             ...
             ((TIMESTAMPDIFF(SECOND, desired_date, validation_date)/86400)/7 + 1
                 ...
                 - (SELECT count(*) FROM TEMP_non_worked_days WHERE jour BETWEEN validation_date AND desired_date AND DAYOFWEEK(jour) = 7)) AS NBSAM,
             (SELECT (
                         SELECT IF(horaire1 IS NOT NULL AND horaire2 IS NOT NULL,
                                   (TIME_TO_SEC(CAST(horaire2 AS TIME)) - TIME_TO_SEC(CAST(horaire1 AS TIME))), 0)
                         FROM TEMP_worked_days
             ...
             ((TIMESTAMPDIFF(SECOND, desired_date, validation_date)/86400)/7 + 1
                 ...
                 - (SELECT count(*) FROM TEMP_non_worked_days WHERE jour BETWEEN validation_date AND desired_date AND DAYOFWEEK(jour) = 1)) AS NBDIM,
             (SELECT (
                         SELECT IF(horaire1 IS NOT NULL AND horaire2 IS NOT NULL,
                                   (TIME_TO_SEC(CAST(horaire2 AS TIME)) - TIME_TO_SEC(CAST(horaire1 AS TIME))), 0)
                         FROM TEMP_worked_days
