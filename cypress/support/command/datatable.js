 ...
 */
Cypress.Commands.add('checkDataInDatatable', (object, objectId = 'label', tableId, propertiesMap) => {
    cy.get(`#${tableId} tbody td`).contains(object[objectId]).then((td) => {
        ...
        cy.wrap(columnIndexes).should('have.keys', Object.keys(object)).then(() => {
            ...
            Object.keys(columnIndexes).forEach((objectProperty) => {
                cy.wrap(td).parent('tr')
                    ...
                    .contains(object[objectProperty]
                        .toString()
                    );
            });
        });
