...
Cypress.Commands.add('addTypeInSettings', (settingsItemName) => {
    ...
    cy
        .get(`[data-menu=${settingsItemName}]`).eq(0)
        .click().wait('@settings_free_field_api');
    cy
        .get(`[data-menu=${settingsItemName}] button.add-entity`)
    ...
    cy
        .get(`[data-menu=${settingsItemName}] .main-entity-content`)
        .then(($div) => {
...
Cypress.Commands.add('editTypeInSettings', (settingsItemName) => {
    ...
    cy
        ...
        .type(labelName);
    cy
        .get('.main-entity-content input[name=description]')
        .clear()
        .type('CypressModify');
...
Cypress.Commands.add('uncheckAllFixedFieldInSettings', (tableName) => {
    ...
    cy
        .get('[data-menu=champs_fixes]').eq(0)
        .click().wait('@settings_fixed_field_api');
    cy
        .get(`[data-menu=champs_fixes] table[id=table-${tableName}-fixed-fields]`).find('input[type=checkbox]')
        .uncheck({force: true});
