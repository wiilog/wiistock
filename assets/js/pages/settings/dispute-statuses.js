...
export function initializeArrivalDisputeStatuses($container, canEdit) {
    initializeDisputeStatuses($container, canEdit, 'arrival');
}

export function initializeReceptionDisputeStatuses($container, canEdit) {
    initializeDisputeStatuses($container, canEdit, 'reception');
}

function initializeDisputeStatuses($container, canEdit, mode) {
    ...
    const tableSelector = `#${mode}-dispute-statuses-table`;
    const route = Routing.generate(`settings_dispute_statuses_api`, {mode})

    const table = EditableDatatable.create(tableSelector, {
        route,
        deleteRoute: `settings_delete_dispute_status`,
