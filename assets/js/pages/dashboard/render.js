...
global.applyStyle = applyStyle;
global.withStyle = withStyle;
...
const creators = {
    ...
    [DAILY_ARRIVALS]: {
        ...
        callbackArguments: {route: `get_arrival_um_statistics`}
    },
    [LATE_PACKS]: {
    ...
    [RECEIPT_ASSOCIATION]: {
        callback: createChart,
        callbackArguments: {route: `get_asso_recep_statistics`}
    },
    ...
    [PACK_TO_TREAT_FROM]: {
        callback: createChart,
        callbackArguments: {cssClass: 'multiple'}
    },
    ...
    [DAILY_HANDLING_INDICATOR]: {
        callback: createIndicatorElement,
        callbackArguments: {route: `handling_index`}
    },
    [DAILY_HANDLING]: {
        callback: createChart
    },
...
function renderComponent(component, $container, data) {
    ...
    if(!creators[component.meterKey]) {
        ...
        const {callback, callbackArguments} = creators[component.meterKey];
    ...
    if (mode === MODE_EDIT
        ...
        return `
            <div class="points has-tooltip" title="${trimmedText}">
                <img src="../../../../public/svg/timeline-information-dashboard.svg" alt="Icône" width="12px">
            </div>
        `;
    }
}
        ...
        <div class="d-flex ${cardSize} p-1">
            <a class="card wii-card request-card pointer p-3 my-2 shadow-sm flex-grow-1 ${cursor} bg-${request.cardColor}" ${link} style="${request.cardBackgroundColor ? (`background-color: ${request.cardBackgroundColor}!important`) : ''}">
                ...
                <div class="wii-card-body">
                    <div class="row">
                        <div class="col-12">
                            ${applyStyle(data, redefinedNumberingConfig, 5, `<p class="card-title no-color text-center text-wrap">${request.requestBodyTitle}</p>`, {}, firstIteration)}
                        </div>
                        <div class="col-12">
